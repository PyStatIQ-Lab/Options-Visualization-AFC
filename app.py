...
def fetch_nifty_price():
    try:
        ...
        st.error(f"Failed to fetch Nifty price: {str(e)}")
    return None

# FETCH OPTIONS DATA
@st.cache_data(ttl=300)
def fetch_options_data(asset_key="NSE_INDEX|Nifty 50", expiry="2025-04-24"):
...
def process_options_data(raw_data, spot_price):
    if not raw_data or 'data' not in raw_data or not spot_price:
        return None
...
def build_stacked_bar(df, metric='oi'):
    ...
    try:
        ...
        st.error(f"Error creating stacked bar: {str(e)}")
        return go.Figure()
...
def build_iv_smile(df, spot_price):
    ...
    try:
        ...
        st.error(f"Error creating IV smile: {str(e)}")
        return go.Figure()
...
def build_gamma_exposure(df, spot_price):
    ...
    try:
        ...
        st.error(f"Error creating gamma exposure chart: {str(e)}")
        return go.Figure()
...
def build_pcr_chart(df, spot_price):
    ...
    try:
        ...
        st.error(f"Error creating PCR chart: {str(e)}")
        return go.Figure()
...
def build_moneyness_chart(df):
    ...
    try:
        ...
        st.error(f"Error creating moneyness chart: {str(e)}")
        return go.Figure()
...
def main():
    ...
    with col1:
        expiry = st.text_input("Expiry Date (YYYY-MM-DD):", "2025-04-24")
    with col2:
    ...
    if spot_price:
        ...
        st.error("Failed to fetch Nifty spot price. Using default value 22000 for calculations.")
        spot_price = 22000  # Default fallback value
